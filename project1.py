...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    """
    Hinge loss on a single data point given specific classification
    parameters.
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    """
    Hinge loss for given classification parameters averaged over a
    given dataset
    ...
    Returns:
        ...
        parameters.  This number is the average hinge loss.
    """
    y = np.dot(feature_matrix,theta)+theta_0
    z = labels*y
    tot_loss = np.maximum(1-z,0.0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for _ in range(T):
        for i in get_order(nsamples):
            ...
            thetaSum += theta
            theta_0Sum += theta_0
    return (thetaSum/(T*nsamples),theta_0Sum/(T*nsamples))
    #raise NotImplementedError
...
def pegasos_single_step_update(
    ...
    Returns:
        ...
        completed.
    """
    pro = np.dot(theta,feature_vector)
    z = (pro + theta_0)*label
    if z<=1 :
        theta = theta + eta*(1*label*feature_vector - L*theta)
